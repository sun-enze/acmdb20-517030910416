...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		Iterator<BTreeEntry> iter = ((BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY)).iterator();
		BTreeEntry curEntry = null;
		while(iter.hasNext()) {
			curEntry = iter.next();
			if(f == null || f.compare(Op.LESS_THAN_OR_EQ, curEntry.getKey())) {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreePageId curPageId = new BTreePageId(page.getId().getTableId(), getEmptyPageNo(tid, dirtypages), BTreePageId.LEAF);
		writePage(new BTreeLeafPage(curPageId, BTreeHeaderPage.createEmptyPageData(), keyField));
		Database.getBufferPool().discardPage(curPageId);
		BTreeLeafPage curPage = (BTreeLeafPage) getPage(tid, dirtypages, curPageId, Permissions.READ_WRITE);

		curPage.setRightSiblingId(page.getRightSiblingId());
		page.setRightSiblingId(curPageId);
		curPage.setLeftSiblingId(page.getId());

		if (curPage.getRightSiblingId() != null)
			((BTreeLeafPage) getPage(tid, dirtypages, curPage.getRightSiblingId(), Permissions.READ_WRITE)).setLeftSiblingId(curPage.getId());

		int numRemoved = page.getNumTuples() / 2;

		Iterator<Tuple> tupleIter = page.iterator();
		int pos = -1;
		for(int i = 0; tupleIter.hasNext(); i++) {
			Tuple curTuple = tupleIter.next();
			if(field.compare(Op.GREATER_THAN_OR_EQ, curTuple.getField(keyField))) {
				pos = i;
			}
			if(i >= numRemoved) {
				page.deleteTuple(curTuple);
				curPage.insertTuple(curTuple);
			}
		}

		Field faField = curPage.iterator().next().getField(keyField);
		BTreePageId faId = page.getParentId();
		if(faId.pgcateg() == BTreePageId.ROOT_PTR) {
			int root = getEmptyPageNo(tid, dirtypages);
			faId = new BTreePageId(page.getId().getTableId(), root, BTreePageId.INTERNAL);
			BTreeRootPtrPage rootPtrPage = (BTreeRootPtrPage) getPage(tid, dirtypages, BTreeRootPtrPage.getId(tableid), Permissions.READ_WRITE);
			rootPtrPage.setRootId(faId);
			dirtypages.put(rootPtrPage.getId(), rootPtrPage);
		}

		BTreeInternalPage faPage = (BTreeInternalPage) getPage(tid, dirtypages, faId, Permissions.READ_WRITE);
		if(faPage.getNumEmptySlots() == 0) {
			dirtypages.put(faPage.getId(), faPage);
			faPage = splitInternalPage(tid, dirtypages, faPage, faField);
		}

		faPage.insertEntry(new BTreeEntry(faField, page.getId(), curPage.getId()));
		page.setParentId(faPage.getId());
		curPage.setParentId(faPage.getId());

		dirtypages.put(curPage.getId(), curPage);
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeInternalPage page, Field field) 
					throws DbException, IOException, TransactionAbortedException {
		BTreePageId curPageId = new BTreePageId(page.getId().getTableId(), getEmptyPageNo(tid, dirtypages), BTreePageId.INTERNAL);
		writePage(new BTreeLeafPage(curPageId, BTreeHeaderPage.createEmptyPageData(), keyField));
		Database.getBufferPool().discardPage(curPageId);
		BTreeInternalPage curPage = (BTreeInternalPage) getPage(tid, dirtypages, curPageId, Permissions.READ_WRITE);

		int numRemoved = page.getNumEntries() / 2;

		Iterator<BTreeEntry> entryIter = page.iterator();
		BTreeEntry midEntry = null;
		int pos = -1;
		for(int i = 0; entryIter.hasNext(); i++) {
			BTreeEntry curEntry = entryIter.next();
			if(field.compare(Op.GREATER_THAN_OR_EQ, curEntry.getKey())) {
				pos = i;
			}
			if(i > numRemoved) {
				page.deleteKeyAndRightChild(curEntry);
				curPage.insertEntry(curEntry);
			}
			else if(i == numRemoved) {
				midEntry = curEntry;
				page.deleteKeyAndRightChild(curEntry);
			}
		}
		...
		if(faPage.getNumEmptySlots() == 0) {
			dirtypages.put(faPage.getId(), faPage);
			faPage = splitInternalPage(tid, dirtypages, faPage, midEntry.getKey());
		}

		faPage.insertEntry(new BTreeEntry(midEntry.getKey(), page.getId(), curPage.getId()));
		page.setParentId(faPage.getId());
		curPage.setParentId(faPage.getId());

		updateParentPointers(tid, dirtypages, curPage);
		updateParentPointers(tid, dirtypages, page);
		updateParentPointers(tid, dirtypages, faPage);
		dirtypages.put(curPage.getId(), curPage);
		dirtypages.put(page.getId(), page);
		dirtypages.put(faPage.getId(), faPage);
		if(pos >= numRemoved) {
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numRemoved = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
		Iterator<BTreeEntry> entryIter = leftSibling.reverseIterator();
		BTreeEntry tmp = new BTreeEntry(parentEntry.getKey(), null, page.iterator().next().getLeftChild());
		for(int i = 0; i < numRemoved && entryIter.hasNext(); i++) {
			BTreeEntry curEntry = entryIter.next();
			tmp.setLeftChild(curEntry.getRightChild());
			page.insertEntry(tmp);
			tmp = new BTreeEntry(curEntry.getKey(), null, curEntry.getRightChild());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			...
			BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException {
		int numRemoved = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
		Iterator<BTreeEntry> entryIter = rightSibling.iterator();
		BTreeEntry tmp = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), null);
		for(int i = 0; i < numRemoved && entryIter.hasNext(); i++) {
			BTreeEntry curEntry = entryIter.next();
			tmp.setRightChild(curEntry.getLeftChild());
			page.insertEntry(tmp);
			tmp = new BTreeEntry(curEntry.getKey(), curEntry.getLeftChild(), null);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeLeafPage leftPage, BTreeLeafPage rightPage, BTreeInternalPage parent, BTreeEntry parentEntry) 
					throws DbException, IOException, TransactionAbortedException {
		Iterator<Tuple> tupleIter = rightPage.iterator();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> entryIter = rightPage.iterator();
