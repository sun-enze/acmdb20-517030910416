...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        int n = enumerateSubsets(joins, 1).size();
        for(int i = 1; i <= n; i++) {
            for(Set<LogicalJoinNode> s : enumerateSubsets(joins, i)) {
                CostCard bestPlan = new CostCard();
                bestPlan.cost = 1e18;
                for(LogicalJoinNode node : s) {
                    CostCard curplan = computeCostAndCardOfSubplan(stats, filterSelectivities, node, s, bestPlan.cost, plans);
                    if(curplan != null && curplan.cost < bestPlan.cost) {
                        bestPlan = curplan;
                    }
                }
                plans.addPlan(s, bestPlan.cost, bestPlan.card, bestPlan.plan);
            }
        }
